(function() {
    ...
    function deepEqual(left, right) {
        ...
        if (Object.keys(left).length !== Object.keys(right).length)
            return false; // if unequal number of elements found they are not
        // equal

        if (typeof (left) !== typeof (right))
            return false; // type of checking for comparision

        if (typeof (left) === "object") { // if type of found to be object we
            ...
            if (left === right)
                return true; // here we reach if the left and right both are
